...
import { categoryLabels, cleaningTypeLabels, spaces } from "@/lib/mockData";
      ...
      <div className="space-y-4">
        ...
        <Card className="p-5 border-border/60 shadow-card">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input defaultValue="Living Colours AR" className="mt-1.5" />
            </div>
            <div>
              <Label>Fuso horário</Label>
              <Input defaultValue="Europe/Lisbon" className="mt-1.5" />
            </div>
          </div>
        </Card>
        ...
        <Card className="p-5 border-border/60 shadow-card">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.values(categoryLabels).map((c) => (
              <span key={c} className="px-3 py-1 bg-accent text-accent-foreground rounded-full text-sm">{c}</span>
            ))}
          </div>
        </Card>
        ...
        <Card className="p-5 border-border/60 shadow-card">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.values(cleaningTypeLabels).map((c) => (
              <span key={c} className="px-3 py-1 bg-secondary text-secondary-foreground rounded-full text-sm">{c}</span>
            ))}
          </div>
        </Card>
        ...
        <div className="text-center text-xs text-muted-foreground pt-4">
          Living Colours Ops · MVP v0.2 · pronto para Lovable Cloud
        </div>
      </div>
    </div>
  );

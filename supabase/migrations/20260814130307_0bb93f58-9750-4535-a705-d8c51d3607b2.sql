...
-- Backfill: quartos ocupados/reservados sem residente atribuído
UPDATE public.rooms r
SET current_resident_id = s.resident_id
FROM (
  SELECT DISTINCT ON (room_id) room_id, resident_id
  FROM public.stays
  WHERE status IN ('confirmed','checked_in') AND room_id IS NOT NULL AND resident_id IS NOT NULL
  ORDER BY room_id, check_in DESC
) s
WHERE r.id = s.room_id AND r.current_resident_id IS NULL;